...
from keras.layers import Layer , GlobalAveragePooling2D , Dense , Multiply , GlobalAveragePooling3D
#---
class se(Layer) :
...
class se3D(Layer) :
    def __init__(self , reduction_ratio=16 , out="channel_scaled") :
        super().__init__()
        self.rr      = reduction_ratio
        self.out     = out
        self.multily = Multiply()
        self.GAP3D   = GlobalAveragePooling3D(keepdims=True)
    def build(self , input_shape) :
        self.hidden = Dense(max(input_shape[-1]//self.rr , 1) , activation="relu" , kernel_initializer="he_normal")
        if self.out == "channel_scaled" or self.out == "channel_weights" :
            self.score = Dense(input_shape[-1] , activation="sigmoid")
